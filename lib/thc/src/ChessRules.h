...
class ChessRules : public ChessPosition
{
  public:
    ...
    void GenLegalMoveList(std::vector<Move>& moves, std::vector<bool>& check,
                          std::vector<bool>& mate, std::vector<bool>& stalemate);

    // Create a list of all legal moves in this position
    void GenLegalMoveList(MOVELIST* list);

    // Create a list of all legal moves in this position, with extra info
    void GenLegalMoveList(MOVELIST* list, bool check[MAXMOVES], bool mate[MAXMOVES],
