...
int main()
{
    ...
    printf("legal1=%s, normal1=%s, legal2=%s, mate2=%s\n", legal1 ? "true" : "false",
           normal1 ? "true" : "false", legal2 ? "true" : "false", mate2 ? "true" : "false");
    if(legal1 && normal1 && legal2 && mate2)
        printf("As expected, all flags true, so both penultimate and final positions are legal, in "
               "the final position White is mated\n");
    else
        printf("Strange(???!), we expected all flags true, meaning both penultimate and final "
               "positions are legal, in the final position White is mated\n");
}
